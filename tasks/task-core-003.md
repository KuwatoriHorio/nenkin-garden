...
> **状態: ✅ 完了**（2026-07-07, iter:15）— `step()` 末尾で `remove_depleted_sugar`（id昇順・
> 決定論）を発火。枯渇砂糖は力学的に不活性のため既存30テストは挙動バイト維持、新規
> `tests/core_003.rs`（4テスト・S9部分集合）で枯渇削除/枯渇前保持/同時枯渇/foraging回帰を検証。
> 実装は nenkin-implementer(sonnet) に自動委譲、検証・keep はオーケストレーター(opus)。
