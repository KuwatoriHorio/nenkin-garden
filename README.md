...
## render-001: ブラウザ対話デモ（WASM）

決定論コアを WebAssembly で実行し、canvas に網を描画。**クリックで砂糖を置ける**対話デモ。
公開: <https://kuwatorihorio.github.io/nenkin-garden/demo/>（トップページからも導線）。

- `render/` レイヤ（設計メモ §2）は **State を読むだけ**（core ← render の一方向依存）。
  外部依存（wasm-bindgen）は別クレート `render-wasm` に隔離し、**core crate は std のみを維持**。
- クリック→セルの写像 `canvas_to_cell` は純関数（native でテスト）。砂糖 op は tick 境界で適用。

再ビルド手順（GNU toolchain・wasm32 ターゲット・`wasm-bindgen` 0.2.126 CLI が必要）:

```sh
cargo build -p render-wasm --target wasm32-unknown-unknown --release
wasm-bindgen --target web --out-dir docs/demo --out-name render_wasm \
  target/wasm32-unknown-unknown/release/render_wasm.wasm
cargo test -p render-wasm    # 写像の純関数 + 決定性/描画非侵襲
```
